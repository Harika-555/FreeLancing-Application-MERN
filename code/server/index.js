...
mongoose.connect('mongodb+srv://229y1a05a7:ivdyZWKbeFjvjujh@cluster0.qrpxufi.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0',{
    ...
    if (!freelancerId || !updateSkills) {
        return res.status(400).json({ error: "freelancerId and updateSkills are required." });
    }
    ...
    try {
        ...
        const newProject = new Project({
            ...
            skills: projectSkills.map(s => s.trim()),
            clientId,
            clientName,
            clientEmail,
            postedDate: new Date()
  ...
  try {
    ...
    if (!freelancer || !freelancerData || !project || !client) {
      return res.status(400).json({ error: 'Invalid data: user, project, or client not found.' });
    }
    ...
    // ❗️ Prevent duplicate bid logic (optional improvement)
